# konkan-railway-live-position-cli

boru-driven command-line client **and** interactive REPL for the KonkanRailwayLivePosition
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/konkan-railway-live-position-cli)
make build

# 2. See usage (words, entities, env vars)
./konkan-railway-live-position-cli --help

# 3. Provide credentials once, via the environment
export KONKAN_RAILWAY_LIVE_POSITION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./konkan-railway-live-position-cli list train
./konkan-railway-live-position-cli load 1 train            # {id:1} shorthand
./konkan-railway-live-position-cli load '{id:1}' train       # explicit match map

# 5. Override the API base URL for a single call
KONKAN_RAILWAY_LIVE_POSITION_BASE=https://api.example.com ./konkan-railway-live-position-cli list train

# 6. No arguments -> interactive REPL
./konkan-railway-live-position-cli
konkan-railway-live-position> list train
konkan-railway-live-position> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/konkan-railway-live-position-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/konkan-railway-live-position-cli list train
   ```
...
```sh
./konkan-railway-live-position-cli load 1 train          # scalar shorthand for {id:1}
./konkan-railway-live-position-cli load '{id:1}' train     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `konkan-railway-live-position>`). Each line is
evaluated as its own boru expression:

```text
$ ./konkan-railway-live-position-cli
konkan-railway-live-position> list train
konkan-railway-live-position> /help
konkan-railway-live-position> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `train`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/konkan-railway-live-position-cli`. |
...
(`./konkan-railway-live-position-cli <expr>`) and interactively (the REPL), and expressions compose the
