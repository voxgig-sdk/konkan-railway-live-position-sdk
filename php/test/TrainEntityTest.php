...
class TrainEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = KonkanRailwayLivePositionSDK::test(null, null);
        $ent = $testsdk->Train(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = KonkanRailwayLivePositionSDK::test($seed, null);
        $seen = iterator_to_array($base->Train(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Train(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = train_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "train." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $train_ref01_data = null;
        if (count($train_ref01_data_raw) > 0) {
...
function train_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/train/TrainTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["train01", "train02", "train03"] as $k) {
