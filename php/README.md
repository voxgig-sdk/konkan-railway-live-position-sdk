# KonkanRailwayLivePosition PHP SDK



The PHP SDK for the KonkanRailwayLivePosition API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Train()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/konkan-railway-live-position-sdk/releases](https://github.com/voxgig-sdk/konkan-railway-live-position-sdk/releases)
...
print_r(array_map(fn($item) => $item->data_get(), $train));
```
...
```php
$client = KonkanRailwayLivePositionSDK::test($testopts, $sdkopts);
```
...
| `Train` | `($data): TrainEntity` | Create a Train entity instance. |
...
```
php/
├── konkanrailwayliveposition_sdk.php          -- Main SDK class
...
The main class (`konkanrailwayliveposition_sdk.php`) exports the SDK class
...
// $train->data_get() now returns the train data from the last list
// $train->match_get() returns the last match criteria
```
