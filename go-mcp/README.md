# konkan-railway-live-position-mcp

[MCP](https://modelcontextprotocol.io) server exposing the KonkanRailwayLivePosition SDK as
two agent tools — `konkan-railway-live-position_list` and `konkan-railway-live-position_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/konkan-railway-live-position-mcp)
make build

# 2. Provide credentials via the environment
export KONKAN_RAILWAY_LIVE_POSITION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user konkan-railway-live-position \
  -- /absolute/path/to/konkan-railway-live-position-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./konkan-railway-live-position-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// konkan-railway-live-position_list: first page of records
{ "entity": "train" }
{ "entity": "train", "query": { } }

// konkan-railway-live-position_load: one record by id
{ "entity": "train", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/konkan-railway-live-position-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user konkan-railway-live-position \
     -- "$PWD"/dist/*/konkan-railway-live-position-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `konkan-railway-live-position_list` and `konkan-railway-live-position_load` tools now appear
   in new sessions. Ask the agent to *"list train using konkan-railway-live-position"*
   and it calls `konkan-railway-live-position_list` with `{"entity":"train"}`.
...
```sh
export KONKAN_RAILWAY_LIVE_POSITION_APIKEY=sk_live_xxx            # API key
export KONKAN_RAILWAY_LIVE_POSITION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `konkan-railway-live-position_list` tool
...
### Call the `konkan-railway-live-position_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `konkan-railway-live-position_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `konkan-railway-live-position_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KONKAN_RAILWAY_LIVE_POSITION_APIKEY` | API key sent with every request. |
| `KONKAN_RAILWAY_LIVE_POSITION_BASE` | Optional override of the API base URL. |
...
```sh
./konkan-railway-live-position-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"konkan-railway-live-position_load","arguments":{"entity":"train","query":{"id":1}}}}'
```
