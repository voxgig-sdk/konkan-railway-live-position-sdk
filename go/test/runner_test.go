...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("KONKANRAILWAYLIVEPOSITION_TEST_LIVE") == "TRUE" ||
		os.Getenv("KONKANRAILWAYLIVEPOSITION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("KONKANRAILWAYLIVEPOSITION_TEST_EXPLAIN"); explain != "" {
