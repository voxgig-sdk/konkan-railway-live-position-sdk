...
import (
	...
	omni "github.com/voxgig-sdk/konkan-railway-live-position-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KonkanRailwayLivePositionError); is {
				out := map[string]any{
					"name":    "KonkanRailwayLivePositionError",
...
func omniCtx(arg any, client *sdk.KonkanRailwayLivePositionSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KonkanRailwayLivePositionSDK); is {
			client = live
