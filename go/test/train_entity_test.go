...
func TestTrainEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Train(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Train(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		trainRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.train")))
...
func trainBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("KONKAN_RAILWAY_LIVE_POSITION_TEST_TRAIN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["KONKAN_RAILWAY_LIVE_POSITION_TEST_TRAIN_ENTID"])
