...
module KonkanRailwayLivePositionUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KonkanRailwayLivePositionResponse.new({})
    ...
    if fetch_err
      ...
      response = KonkanRailwayLivePositionResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KonkanRailwayLivePositionResponse.new(fetched)
    else
