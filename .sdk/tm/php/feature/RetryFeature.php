...
// KonkanRailwayLivePosition SDK retry feature
...
class KonkanRailwayLivePositionRetryFeature extends KonkanRailwayLivePositionBaseFeature
{
    ...
    public function init(KonkanRailwayLivePositionContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (KonkanRailwayLivePositionContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(KonkanRailwayLivePositionContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(KonkanRailwayLivePositionContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
