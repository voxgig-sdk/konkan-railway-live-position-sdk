...
//    only - a typed KonkanRailwayLivePositionContext there would make every ctx
...
final class KonkanRailwayLivePositionOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new KonkanRailwayLivePositionContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new KonkanRailwayLivePositionSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new KonkanRailwayLivePositionResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new KonkanRailwayLivePositionError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new KonkanRailwayLivePositionResponse($clean['response']);
    ...
    public static function err_from_map($m): ?KonkanRailwayLivePositionError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new KonkanRailwayLivePositionError(is_string($code) ? $code : '', $msg);
    }
