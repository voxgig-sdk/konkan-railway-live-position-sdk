...
// KonkanRailwayLivePosition SDK feature test
//
...
class FtCtrl extends KonkanRailwayLivePositionControl
{
...
class FtClient
{
    ...
    private KonkanRailwayLivePositionUtility $utility;

    public function __construct(KonkanRailwayLivePositionUtility $utility, array $options)
    {
    ...
    public function get_utility(): KonkanRailwayLivePositionUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KonkanRailwayLivePositionContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KonkanRailwayLivePositionContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KonkanRailwayLivePositionContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KonkanRailwayLivePositionConfig::make_feature($name);
    ...
    private static function buildUrl(KonkanRailwayLivePositionSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KonkanRailwayLivePositionError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KonkanRailwayLivePositionResponse($fetched);
        }
    ...
    private function fail(KonkanRailwayLivePositionContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KonkanRailwayLivePositionContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
