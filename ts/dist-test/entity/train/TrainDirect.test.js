...
(0, node_test_1.describe)('TrainDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.KonkanRailwayLivePositionSDK({
    ...
    (0, node_test_1.test)('direct-load-train', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-train', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-train', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-train', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.KONKAN_RAILWAY_LIVE_POSITION_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.KonkanRailwayLivePositionSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['KONKAN_RAILWAY_LIVE_POSITION_TEST_TRAIN_ENTID'];
    ...
    const client = new __1.KonkanRailwayLivePositionSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=TrainDirect.test.js.map