...
//     (\`new KonkanRailwayLivePositionSDK(...)\` / \`KonkanRailwayLivePositionSDK.test(...)\` become
//     \`KonkanRailwayLivePositionSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'KonkanRailwayLivePositionSDK'
...
const TEST_SEED = {"entity":{"train":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new KonkanRailwayLivePositionSDK(...)\` and any
// \`KonkanRailwayLivePositionSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KonkanRailwayLivePositionSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KonkanRailwayLivePositionSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
