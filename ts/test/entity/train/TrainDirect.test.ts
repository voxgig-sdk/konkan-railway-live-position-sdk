...
describe('TrainDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new KonkanRailwayLivePositionSDK({
  ...
  test('direct-load-train', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-train', setup.live)) return
  ...
  test('direct-list-train', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-train', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['KONKAN_RAILWAY_LIVE_POSITION_TEST_TRAIN_ENTID']
