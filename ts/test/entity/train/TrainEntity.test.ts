...
describe('TrainEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'train.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set KONKAN_RAILWAY_LIVE_POSITION_TEST_TRAIN_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['KONKAN_RAILWAY_LIVE_POSITION_TEST_TRAIN_ENTID']
