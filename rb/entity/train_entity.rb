...
class TrainEntity
  ...
  def make
    opts = @_entopts.dup
    TrainEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = KonkanRailwayLivePositionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Train, Hash] the current Train data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = KonkanRailwayLivePositionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Train fields)
  def match_get
  ...
  # Load a single Train.
  #
  # @param reqmatch [TrainLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Train.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Train, Hash] the loaded Train; raises KonkanRailwayLivePositionError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = KonkanRailwayLivePositionHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [TrainListMatch, Hash, nil] match filter (any subset of
  #   Train fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Train>, Array] the matching Train items; raises KonkanRailwayLivePositionError on failure
