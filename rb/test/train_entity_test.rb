...
class TrainEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = KonkanRailwayLivePositionSDK.test(seed, nil)
    seen = base.Train(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Train(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = train_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "train." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set KONKAN_RAILWAY_LIVE_POSITION_TEST_TRAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    train_ref01_data = nil
    if train_ref01_data_raw.length > 0
...
def train_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "train", "TrainTestData.json")
  ...
  entid_env_raw = ENV["KONKAN_RAILWAY_LIVE_POSITION_TEST_TRAIN_ENTID"]
  ...
  if env["KONKAN_RAILWAY_LIVE_POSITION_TEST_LIVE"] == "TRUE"
    ...
    client = KonkanRailwayLivePositionSDK.new(Helpers.to_map(merged_opts))
  end
