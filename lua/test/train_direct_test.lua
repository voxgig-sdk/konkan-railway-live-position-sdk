...
describe("TrainDirect", function()
  it("should direct-list-train", function()
    local setup = train_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-train", setup.live and "live" or "unit")
  ...
  it("should direct-load-train", function()
    local setup = train_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-train", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function train_direct_setup(mockres)
