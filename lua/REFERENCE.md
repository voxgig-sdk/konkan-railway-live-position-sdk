# KonkanRailwayLivePosition Lua SDK Reference

Complete API reference for the KonkanRailwayLivePosition Lua SDK.
...
```lua
local results, err = client:Train(nil):list(nil, nil)
```
...
```lua
local result, err = client:Train(nil):load({ id = "train_id" }, nil)
```
