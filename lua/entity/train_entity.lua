...
local TrainEntity = {}
TrainEntity.__index = TrainEntity


function TrainEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TrainEntity)
  self._name = "train"
  self._client = client
...
function TrainEntity:get_name()
  return self._name
end


function TrainEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TrainEntity.new(self._client, opts)
end
...
function TrainEntity:mark_deleted()
  self._deleted = true
end


function TrainEntity:deleted()
  return true == self._deleted
end


function TrainEntity:data_set(args)
  if args ~= nil then
...
function TrainEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TrainEntity:match_set(args)
  if args ~= nil then
...
function TrainEntity:match_get()
...
function TrainEntity:stream(action, args, callopts)
...
function TrainEntity:_run_op(ctx, post_done)
  local utility = self._utility
