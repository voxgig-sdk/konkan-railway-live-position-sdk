# KonkanRailwayLivePosition SDK test runner
...
class KonkanRailwayLivePositionTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KonkanRailwayLivePositionTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KonkanRailwayLivePositionTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KonkanRailwayLivePositionTestRunner.getenv("KONKAN_RAILWAY_LIVE_POSITION_TEST_LIVE")
        override = KonkanRailwayLivePositionTestRunner.getenv("KONKAN_RAILWAY_LIVE_POSITION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KonkanRailwayLivePositionTestRunner.getenv(key)
        ...
        explain = KonkanRailwayLivePositionTestRunner.getenv("KONKAN_RAILWAY_LIVE_POSITION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if KonkanRailwayLivePositionTestRunner._test_control is not None:
            return KonkanRailwayLivePositionTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KonkanRailwayLivePositionTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KonkanRailwayLivePositionTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KonkanRailwayLivePositionTestRunner.load_test_control()
...
def load_env_local():
    KonkanRailwayLivePositionTestRunner.load_env_local()


def env_override(m):
    return KonkanRailwayLivePositionTestRunner.env_override(m)


def entity_data(v):
    return KonkanRailwayLivePositionTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KonkanRailwayLivePositionTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KonkanRailwayLivePositionTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KonkanRailwayLivePositionTestRunner.load_test_control()


def live_client_options():
    return KonkanRailwayLivePositionTestRunner.live_client_options()


def live_delay_ms():
    return KonkanRailwayLivePositionTestRunner.live_delay_ms()
