...
from konkanrailwayliveposition_sdk.konkanrailwayliveposition_types import (
    Train,
    TrainLoadMatch,
    TrainListMatch,
)
...
class TrainEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TrainEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Train:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Train().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Train]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Train().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
